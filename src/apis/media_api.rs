...
/// struct for typed errors of method [`delete_media_file`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
pub async fn delete_media_file(configuration: &configuration::Configuration, media_id: &str, is4k: Option<bool>) -> Result<(), Error<DeleteMediaFileError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_media_id = media_id;
    let p_query_is4k = is4k;

    let uri_str = format!("{}/media/{mediaId}/file", configuration.base_path, mediaId=crate::apis::urlencode(p_path_media_id));
    let mut req_builder = configuration.client.request(reqwest::Method::DELETE, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<DeleteMediaFileError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
